...
{
    public class FastBoundingFrustum
    {
        private readonly Vector3 NearNormal, LeftNormal, RightNormal, BottomNormal, TopNormal; 
        private readonly float NearD, LeftD, RightD, BottomD, TopD; 
        ...
        public bool Intersects(ref BoundingBox box) { 
            ...
            p.X = (NearNormal.X >= 0 ? box.Minimum.X : box.Maximum.X); 
            p.Y = (NearNormal.Y >= 0 ? box.Minimum.Y : box.Maximum.Y); 
            p.Z = (NearNormal.Z >= 0 ? box.Minimum.Z : box.Maximum.Z); 
            if(NearD + (NearNormal.X * p.X) + (NearNormal.Y * p.Y) + (NearNormal.Z * p.Z) > 0) return false; 
 
            p.X = (LeftNormal.X >= 0 ? box.Minimum.X : box.Maximum.X); 
            p.Y = (LeftNormal.Y >= 0 ? box.Minimum.Y : box.Maximum.Y); 
            p.Z = (LeftNormal.Z >= 0 ? box.Minimum.Z : box.Maximum.Z); 
            if(LeftD + (LeftNormal.X * p.X) + (LeftNormal.Y * p.Y) + (LeftNormal.Z * p.Z) > 0) return false; 
 
            p.X = (RightNormal.X >= 0 ? box.Minimum.X : box.Maximum.X); 
            p.Y = (RightNormal.Y >= 0 ? box.Minimum.Y : box.Maximum.Y); 
            p.Z = (RightNormal.Z >= 0 ? box.Minimum.Z : box.Maximum.Z); 
            if(RightD + (RightNormal.X * p.X) + (RightNormal.Y * p.Y) + (RightNormal.Z * p.Z) > 0) return false; 
 
            p.X = (BottomNormal.X >= 0 ? box.Minimum.X : box.Maximum.X); 
            p.Y = (BottomNormal.Y >= 0 ? box.Minimum.Y : box.Maximum.Y); 
            p.Z = (BottomNormal.Z >= 0 ? box.Minimum.Z : box.Maximum.Z); 
            if(BottomD + (BottomNormal.X * p.X) + (BottomNormal.Y * p.Y) + (BottomNormal.Z * p.Z) > 0) return false; 
 
            p.X = (TopNormal.X >= 0 ? box.Minimum.X : box.Maximum.X); 
            p.Y = (TopNormal.Y >= 0 ? box.Minimum.Y : box.Maximum.Y); 
            p.Z = (TopNormal.Z >= 0 ? box.Minimum.Z : box.Maximum.Z); 
