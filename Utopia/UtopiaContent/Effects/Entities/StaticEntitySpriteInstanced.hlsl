...
struct VSInput
{
	...
	float2 TexCoord			    : TEXCOORD;  //Texture Coordinate at vertex
	//Instanced properties
    matrix WorldMatrix			: TRANSFORM; //World Matrix (World Position + Scaling)
    float4 Color			    : COLOR;	 //Environment color that will affect Sprite (Light sources ...)
	uint TexIndex			    : TEXINDEX;  //Texture ID
};
...
PSInput VS (VSInput input)
{
	...
	worldPosition = mul(mul(worldPosition, input.WorldMatrix), WorldFocus);

	output.Position = mul(worldPosition, ViewProjection);
	float3 textureMap = {input.TexCoord, input.TexIndex};
	output.UVW = textureMap;
