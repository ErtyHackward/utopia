...
namespace Utopia.Shared.Server.Managers
{
    ...
    /// </summary>
    public abstract class MonoSqliteStorage : IDisposable
    {
        ...
        protected MonoSqliteStorage(string fileName, bool wipeDatabase = false)
        {
        ...
        public int InsertBlob(string query, byte[] blob)
        {
            lock (_syncRoot)
            {
                ...
                {
                    logger.Warn<ConnectionState>("Trying to insert Blob Data while the connection status is not open or disposed : {0}", Connection.State);
                    return 0;
                }
            }
        }
