...
namespace Utopia.Shared.World.Processors.Utopia.LandformFct
{
    public class Plain : ITerrainGenerator
    {
        ...
        {       
            ...
            INoise plain_shape_fractal = new FractalFbm(new Simplex(_seed), 2, 3, enuBaseNoiseRange.ZeroToOne);
            //Rescale + offset the output result ==> Wil modify the Scope of output range value
            INoise plain_scale = new ScaleOffset(plain_shape_fractal, 0.20 * _groundGradientTyped.AdjustY, 0.05 * _groundGradientTyped.AdjustY); 
