...
public class D3D9Renderer : CeGui.Renderer {
  ...
  /// </param>
  public D3D9Renderer(D3D.Device d3dDevice, int maxQuads) {
    this.d3dDevice = d3dDevice;

    this.maxVertices = maxQuads * 6; // vertices for two triangles in triangle list mode
    this.vertices = new D3D.CustomVertex.TransformedColoredTextured[this.maxVertices];
    this.textures = new List<D3D9Texture>();
  ...
  private void createVertexBuffer() {
    ...
    this.vertexBuffer = new D3D.VertexBuffer(
      typeof(D3D.CustomVertex.TransformedColoredTextured),
      this.maxVertices,
      this.d3dDevice,
      D3D.Usage.Dynamic | D3D.Usage.WriteOnly,
      D3D.CustomVertex.TransformedColoredTextured.Format,
      D3D.Pool.Default
    );
  ...
  public void PreD3DReset() {
    ...
    foreach(D3D9Texture texture in this.textures)
      texture.PreD3DReset();
  ...
  public override void AddQuad(
    ...
    D3D.BaseTexture d3dTexture = (texture as D3D9Texture).D3DTexture;

    // Is this a quad we should render directly?
    if(!IsQueueingEnabled) {

      // Generate the required vertices
      D3D.CustomVertex.TransformedColoredTextured[] tempVertices =
        new Microsoft.DirectX.Direct3D.CustomVertex.TransformedColoredTextured[6];
      ...
      d3dDevice.VertexFormat = D3D.CustomVertex.TransformedColoredTextured.Format;
      d3dDevice.SetTexture(0, d3dTexture);
      d3dDevice.DrawUserPrimitives(D3D.PrimitiveType.TriangleList, 2, tempVertices);
  ...
  public override void DoRender() {
    D3D.VertexFormats vertexFormat = D3D.CustomVertex.TransformedColoredTextured.Format;
    int stride = D3D.CustomVertex.TransformedColoredTextured.StrideSize;
    ...
    do {
      ...
      for(int index = firstOperation; index <= lastOperation; ++index) {
        ...
        this.d3dDevice.DrawPrimitives(D3D.PrimitiveType.TriangleList, drawStart, drawCount / 3);
      }
  ...
  private void generateQuadVertices(
    D3D.CustomVertex.TransformedColoredTextured[] target, int startIndex,
    ...
    // First triangle
    target[startIndex + 0].X = destRect.Left;
    target[startIndex + 0].Y = destRect.Top;
    target[startIndex + 0].Z = z;
    target[startIndex + 0].Color = colors.topLeft.ToARGB();
    target[startIndex + 0].Tu = textureRect.Left;
    target[startIndex + 0].Tv = textureRect.Top;

    target[startIndex + 1].X = destRect.Left;
    target[startIndex + 1].Y = destRect.Bottom;
    target[startIndex + 1].Z = z;
    target[startIndex + 1].Color = colors.bottomLeft.ToARGB();
    target[startIndex + 1].Tu = textureRect.Left;
    target[startIndex + 1].Tv = textureRect.Bottom;

    target[startIndex + 2].X = destRect.Right;
    target[startIndex + 2].Y = destRect.Bottom;
    target[startIndex + 2].Z = z;
    target[startIndex + 2].Color = colors.bottomRight.ToARGB();
    target[startIndex + 2].Tu = textureRect.Right;
    target[startIndex + 2].Tv = textureRect.Bottom;
    ...
    target[startIndex + 5].X = destRect.Right;
    target[startIndex + 5].Y = destRect.Top;
    target[startIndex + 5].Z = z;
    target[startIndex + 5].Color = colors.topRight.ToARGB();
    target[startIndex + 5].Tu = textureRect.Right;
    target[startIndex + 5].Tv = textureRect.Top;
  ...
  public override Texture CreateTexture() {
    D3D9Texture newTexture = new D3D9Texture(this);
  ...
  public override SizeF Size {
    get {
      return new SizeF(d3dDevice.Viewport.Width, d3dDevice.Viewport.Height);
    }
  }
  ...
  public override Rect Rect {
    get {
      return new Rect(
        0, 0,
        d3dDevice.Viewport.Width, d3dDevice.Viewport.Height
      );
    }
  }
  ...
  private class RenderOperation {
    ...
    public RenderOperation(int startVertex, D3D.BaseTexture texture) {
  ...
  private D3D.VertexBuffer vertexBuffer;
  /// <summary>List of all textures this renderer has created</summary>
  private List<D3D9Texture> textures;
  ...
  private D3D.CustomVertex.TransformedColoredTextured[] vertices;
