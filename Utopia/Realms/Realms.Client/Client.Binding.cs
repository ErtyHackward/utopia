...
namespace Realms.Client
{
    public partial class GameClient
    {
        public void IocBinding(string WindowsCaption, Size windowStartingSize, Size resolutionSize = default(Size))
        {
            ...
            _iocContainer.Bind<ICameraFocused>().To<FirstPersonCameraWithFocus>().InScope(x => GameScope.CurrentGameScope).Named("FirstPCamera").WithConstructorArgument("nearPlane", 0.1f).WithConstructorArgument("farPlane", 2500f); //Type of camera used
            _iocContainer.Bind<ICameraFocused>().To<ThirdPersonCameraWithFocus>().InScope(x => GameScope.CurrentGameScope).Named("ThirdPCamera").WithConstructorArgument("nearPlane", 0.1f).WithConstructorArgument("farPlane", 2500f); //Type of camera used
            ...
            _iocContainer.Bind<StaggingBackBuffer>().ToSelf().InScope(x => GameScope.CurrentGameScope).Named("SolidBuffer").WithConstructorArgument("Name", "GroundBuffer");
