...
public class GuiSystem {
  ...
  */
  ...
  /// <returns>Returns true if the event was handled, and false if it is not.</returns>
  private bool MouseMoveCommon(MouseEventArgs e) {
    Window destWindow = GetTargetWindow(e.Position);
    ...
    if(destWindow != null) {
      ...
      // if mouse didn't actually move, don't send OnMouseMove() messages to windows
      if ((e.MoveDelta.X == 0.0f) && (e.MoveDelta.Y == 0.0f)) {
          e.Handled = true;
      } else {
        while(!e.Handled && (destWindow != null)) {
