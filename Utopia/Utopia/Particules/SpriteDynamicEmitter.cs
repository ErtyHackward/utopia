...
{
    public class SpriteDynamicEmitter : BaseComponent, IEmitter
    {
        ...
        public void FTSUpdate(GameTime timeSpend)
        {
            ...
            _particules.RemoveAll(x => x.Age > x.maxAge); //Remove particules end of life
        }
        ...
        public void EmitParticule(DynamicEntityParticule particuleMetaData,
                                  Vector3D EmittedPosition,
                                  int ParticulesAmount,
                                  Vector3 velocity)
        {
            int nbr = ParticulesAmount;

            Vector3D startUpPosition = EmittedPosition;

            //Emit particules
            while (nbr > 0)
            {
                //Randomize the Velocity
                velocity.X = velocity.X + _rnd.NextFloat(particuleMetaData.EmitVelocityRandomness.X);
                velocity.Y = velocity.Y + _rnd.NextFloat(particuleMetaData.EmitVelocityRandomness.Y);
                velocity.Z = velocity.Z + _rnd.NextFloat(particuleMetaData.EmitVelocityRandomness.Z);
                ...
                {
                    particuleMetaData.AccelerationForce += _weather.Wind.WindFlowFlat * 0.05f; 
                }

                _particules.Add(new SpriteParticule()
                {
                    AccelerationForce = new Vector3D(particuleMetaData.AccelerationForce),
                    ...
                    Position = new FTSValue<Vector3D>(EmittedPosition),
                    Size = particuleMetaData.Size,
                    SizeGrowSpeed = 0,
                    Velocity = velocity,
                    alphaFadingPowBase = 0
                });                 
                nbr--;
            }
        }
