...
namespace Utopia.Worlds.SkyDomes.SharedComp
{
    ...
    public partial class Clouds
    {
        ...
        private void Initialize2D()
        {
            ...
            {
                X = AbstractChunk.ChunkSize.X * _worldParam.VisibleChunkInWorld.X,
                Y = AbstractChunk.ChunkSize.Y,
                Z = AbstractChunk.ChunkSize.Z * _worldParam.VisibleChunkInWorld.Y,
            };

            _nbrLayer = 1; 
            _cloudMap2D = ShaderResourceView.FromFile(_d3dEngine.Device, ClientSettings.TexturePack + @"Weather\clouds.png");
        }

        private void LoadContent2D(DeviceContext context)
        {
            Init2D(context);
        }
        ...
        private void Init2D(DeviceContext Context)
        {
            ...
            _cloudEffect2D.CloudTexture.Value = _cloudMap2D;
            ...
            _cloudIB2D = new IndexBuffer<ushort>(_d3dEngine.Device, 6, SharpDX.DXGI.Format.R16_UInt, "_cloudIB");
            _cloudIB2D.SetData(Context, _ib);
        }


        private void Update2D(GameTime timeSpend)
        {
        }

        private void Interpolation2D(double interpolation_hd, float interpolation_ld, long timePassed)
        {
        }

        private void Draw2D(DeviceContext context, int index)
        {
        ...
        private void Draw2D()
        {
            ...
            if (_cloudVB2D != null)
            {
                ...
                _cloudIB2D.SetToDevice(_d3dEngine.ImmediateContext, 0);
                _d3dEngine.ImmediateContext.DrawIndexed(_cloudIB2D.IndicesCount, 0, 0);
            }
        }

        private void BeforeDispose2D()
        {
            _cloudMap2D.Dispose();
