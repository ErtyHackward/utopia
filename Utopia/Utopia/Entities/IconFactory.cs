    ...
    public class IconFactory : GameComponent
    {
        ...
        private List<Texture2D> Create3DBlockIcons(DeviceContext context, ShaderResourceView cubesTexture, int iconSize)
        {
            ...
            foreach (BlockProfile profile in _visualWorldParameters.WorldParameters.Configuration.GetAllCubesProfiles())
            {
                ...
                MaterialChangeMapping[0] = (byte)profile.Tex_Back.TextureArrayId; //Change the Back Texture Id
                MaterialChangeMapping[1] = (byte)profile.Tex_Front.TextureArrayId; //Change the Front Texture Id
                MaterialChangeMapping[2] = (byte)profile.Tex_Bottom.TextureArrayId; //Change the Bottom Texture Id
                MaterialChangeMapping[3] = (byte)profile.Tex_Top.TextureArrayId; //Change the Top Texture Id
                MaterialChangeMapping[4] = (byte)profile.Tex_Left.TextureArrayId; //Change the Left Texture Id
                MaterialChangeMapping[5] = (byte)profile.Tex_Right.TextureArrayId; //Change the Right Texture Id
