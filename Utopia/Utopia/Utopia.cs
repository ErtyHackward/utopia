...
namespace Utopia
{
    ...
    public class Utopia : Game
    {
        ...
        {
            TerraWorld.ChunkGridSize = (int)Config.Configs["WorldParameters_WorldSize"];
            TerraWorld.ChunkGridSurface = TerraWorld.ChunkGridSize * TerraWorld.ChunkGridSize;
            TerraWorld.Worldsize = new Location3<int>(TerraWorld.Chunksize * TerraWorld.ChunkGridSize, 128, TerraWorld.Chunksize * TerraWorld.ChunkGridSize); 
        }
        ...
        private void KeyboardStateHandling()
        {
            ...
            if (InputHandler.PrevKeyboardState.IsKeyDown(Config.DebugMode) && InputHandler.PrevKeyboardState.IsKeyDown(Keys.LControlKey) && InputHandler.CurKeyboardState.IsKeyUp(Config.DebugMode) && InputHandler.CurKeyboardState.IsKeyDown(Keys.LControlKey))
            {
                FixedTimeSteps = !FixedTimeSteps;
                GameConsole.Write("FixeTimeStep Mode : " + FixedTimeSteps.ToString());
            }

            if (InputHandler.IsKeyPressed(Config.FullScreen)) isFullScreen = !isFullScreen; //Go full screen !
            ...
            if (InputHandler.IsKeyPressed(Config.LockMouseCursor))
            {
                UnlockedMouse = !UnlockedMouse;
            }

            if (InputHandler.IsKeyPressed(Config.VSync))
            {
                VSync = !VSync;
            }

            if (InputHandler.IsKeyPressed(Config.DebugInfo))
            {
                _debugInfo.Activated = !_debugInfo.Activated;
            }

            if (InputHandler.IsKeyPressed(Config.Console)) GameConsole.Show = !GameConsole.Show;
